...
### Setup your VM Network

Because whoever created the template did not do it correctly the first time around. 🤷

::: {.callout-note}
These commands are a pain to type by hand. Use the tab key extensively to autocomplete.
:::

1. There is an example network configuration file that tells your system to configure the _wired_ interfaces using DHCP. You will learn all about DHCP in a later lesson. For now, link to this file to correctly configure your system.

```sh
# create the symbolic link to the example network configuration file
sudo ln -s /usr/lib/systemd/network/89-ethernet.network.example /etc/systemd/network/89-ethernet.network

# verify the contents of the file
cat /etc/systemd/network/89-ethernet.network

## restart the network service to apply the new configuration
sudo systemctl restart systemd-networkd.service
```

1. Now check your VM's IP address with `ip addr`. Note down this address and keep track of it.

### SSH from Windows (not WSL!)

::: {.callout-note}
Run SSH PowerShell or Windows, not WSL. WSL cannot access your EECSNet VPN.
:::

From a Windows Terminal (PowerShell), run the following command:

```sh
ssh ubuntu@<VM_IP_ADDRESS> # e.g. ssh ubuntu@10.3.50.X
```
...
- Grab your public key:
    ...
    ```sh
    cat id_ed25519.pub # or cat id_*.pub
    ```
